...
| [no-local-dependencies](/rules/no-local-dependencies)                                     | Requires that dependencies do not use local file paths, which will likely result in errors when installing from a registry.                                                                  |    |    |    |
...
| [order-properties](/rules/order-properties)                                               | Enforces that package properties are declared in a consistent order.                                                                                                                         | 🎨 | 🔧 |    |
| [prefer-rolling-workspace-spec](/rules/prefer-rolling-workspace-spec)                     | Require that dependencies declared with workspace protocol use a rolling workspace spec instead of a specific semver range.                                                                  |    |    | 💡 |
...
| [require-package-json-export](/rules/require-package-json-export)                         | Ensures that if a package specifies `exports` (or `publishConfig.exports`), an explicit `package.json` export is included.                                                                   |    |    | 💡 |
...
| [restrict-dist-tags](/rules/restrict-dist-tags)                                           | Restricts dependency dist-tags to an allowed list and selected dependency types.                                                                                                             |    |    |    |
