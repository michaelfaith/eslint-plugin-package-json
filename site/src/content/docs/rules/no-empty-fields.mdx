...
<Aside>
  `files` is included by default since an empty array value is meaningfully different than not
  including it at all. See [`no-redundant-files`](/rules/no-redundant-files) for more.
</Aside>
