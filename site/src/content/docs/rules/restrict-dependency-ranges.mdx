...
`type RangeType = 'caret' | '^' | 'pin' | 'tilde' | '~' | 'lt' | '<' | 'le' | '<=' | 'gt' | '>' | 'ge' | '>=';`
...
`rangeType` can be specified using either symbols (eg. `^`, `~`) or names (eg. `"caret", `"tilde"`), with the exception of `"pin"` which does not have an associated symbol.
...
- [`restrict-dist-tags`](/rules/restrict-dist-tags) - Restricts the use of npm dist-tags in dependency declarations.
