...
<Aside type="tip">
  This rule is especially useful in monorepos with many `package.json` files that would ideally be
...
<Aside>
  The default value for `order` changed from `"legacy"` to `"sort-package-json"` in v0.6.0. `legacy`
  order will be removed in the next major release.
</Aside>
